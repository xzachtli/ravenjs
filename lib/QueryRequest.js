...
QueryRequest.prototype.transformWith = function(resultsTransformer) {
	var self = this;
	if (!_(resultsTransformer).isString()) throw new Error('Expected a valid string for results transformer.');
	self.queryData.resultsTransformer = resultsTransformer;
	return self;
};

QueryRequest.prototype.waitForNonStaleResultsAsOf = function(cutoff, waitTimeout) {
	var self = this;
	if (!_(cutoff).isDate()) throw new Error('Expected a valid date for cutoff.');
	if (!!waitTimeout && !_(waitTimeout).isNumber()) throw new Error('Invalid wait timeout specified');
	if (!waitTimeout) waitTimeout = 15;
	self.queryData.cutoff = cutoff.toISOString().replace(/Z$/, '0000');
	self.waitTimeout = waitTimeout;
	return self;
};

QueryRequest.prototype.waitForNonStaleResultsAsOfNow = function(waitTimeout) {
	var self = this;
	return self.waitForNonStaleResultsAsOf(new Date());
};
...
QueryRequest.prototype.results = function(callback) {
	var self = this,
		...
		cutoff: self.queryData.cutoff,
		cutoffEtag: self.queryData.cutoffEtag,
		resultsTransformer: self.queryData.resultsTransformer
	};
	...
	if (_(self.waitTimeout).isNumber()) self.waitTimeout = parseInt(self.waitTimeout);
	...
	if (!queryStrObj.cutoff) delete queryStrObj.cutoff;
	if (!queryStrObj.cutoffEtag) delete queryStrObj.cutoffEtag;
	if (!queryStrObj.resultsTransformer) delete queryStrObj.resultsTransformer;
		...
		self.sendGet(path, function(error, response, data) {
			...
			if (!!self.waitTimeout) {
				if (data.IsStale) {
					...
					if (diff > self.waitTimeout * 1000) {
						if (deferred) return deferred.reject(new Error(format('Waited for %sms for the query to return non stale result.', diff)));
						return callback(serverError, data);
					} else {
