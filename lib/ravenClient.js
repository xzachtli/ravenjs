...
RavenClient.prototype.get = function(id, fn) {
	var self = this;
	request(self.settings, { url: buildUrl.call(self, id) }, function(error, response, body) {
		if (error) return fn(error);
		if (response.statusCode !== 200) return fn(new Error('Error getting document ' + id + '. Server returned status ' + response.statusCode));
		if (!body) return fn(new Error('Error getting document ' + id + '. No data returned by server.'));
		
		return fn(null, parseRavenDocument(id, response, body));
	});
};
...
RavenClient.prototype.save = function(id, doc, fn) {
	...
	if (!id) {
		id = self.settings.idFinder(doc);
		if (!id) id = self.settings.idGenerator(self, doc);
	}
};
