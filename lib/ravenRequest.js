...
RavenRequest.prototype.sendPost = function(opts, callback) {
	...
	self.sendRequest(requestData, function (error, response, data) {
		if (error) return callback (error);
		if (response.statusCode !== 201) return callback (new Error (format('Error posting to %s. Host returned a %d response', opts.path, response.statusCode)));
...
RavenRequest.prototype.sendDelete = function(path, callback) {
	...
	self.sendRequest(requestData, function(error, response, data) {
		if (error) return callback (error);
		if (response.statusCode !== 204) return callback (new Error (format('Error deleting %s. Host returned a %d response.', path, response.statusCode)), response);
