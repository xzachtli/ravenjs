...
describe('DocumentRequests', function() {
	...
	describe('.get', function() {
		...
		it('should throw when callback is undefined', function() {
			expect(function() { request.get('foo'); }).toThrow();
			expect(function() { request.get('foo', null); }).toThrow();
			expect(function() { request.get('foo', undefined); }).toThrow();
		});
		...
		it('should return server error if response code is not 200', function(done) {
			...
			request.get('foo', function(error, data) {
				expect(error).toBeDefined();
				expect(~error.message.indexOf('405')).toBeTruthy();
				ravendb.done();
				done();
			});
		});
